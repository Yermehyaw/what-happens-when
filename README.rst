...
- This serial signal is then decoded at the computer's host USB controller, and
  ...
  device driver. 
The decoding of serial signals also known as input reports, by the host USB controller involves several steps:
These serial signals are sent in a structured and serialized manner, which means that the intial bits of the packets which are now serial signals are sent one after another over the USB connection.
...
*Lastly,Data Transfer: Once the packets are successfully formed and verified, they are passed to the operating system for interpretation by the keyboard's universal HID driver which must have been previously installed on the user's PC.
